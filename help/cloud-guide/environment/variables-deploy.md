...
- `false`—`Consumers` procesan los mensajes disponibles en la cola, cierran la conexión TCP y finalizan. `Consumers` no espere a que otros mensajes entren en la cola, incluso si el número de mensajes procesados es inferior al valor `max_messages` especificado en la variable de implementación `CRON_CONSUMERS_RUNNER`.
